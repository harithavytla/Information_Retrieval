...
// binarized MNB with feature selection using CHI
// Usage: java IR8A numberOfSelectedFeatures

import java.io.*;
import java.util.*;

public class IR8ACHI{
 ...
 public IR8ACHI(int k){ numberOfSelectedFeatures = k; }
 ...
 void readInvertedIndexWithoutTfs(String filename, int c){
  ...
  void computeCHI(String t, int n){
	...
	double Nx1 = Nc[0], Nx0 = Nc[1];
      	...
      	double CHI = ( double ) (N *(N11*N00 - N10 * N01)*(N11 * N00 -N10 * N01))/(Nx1 * N1x * Nx0 * N0x);
		sorted.put(CHI + n * 0.00001, t);
  } 
  ...
  void selectFeatures(){
	...
	n = 0;
	features.forEach(x -> computeCHI(x, n++));
  ...
  void accuracy(){
	...
	System.out.println(testTP + " " + testFP + " "
		+ testFN + " " + testTN);
	System.out.println("training accuracy using chisquare ="+ (double)(trainingTP+trainingTN)/(trainingTP +  trainingFP + trainingFN + trainingTN));
	System.out.println("testing accuracy using chisquare="+ (double)(testTP + testTN)/(testTP +  testFP + testFN + testTN));
  }
 ...
 public static void main(String[] args){
   ...
   IR8ACHI ir8 = new IR8ACHI(Integer.parseInt(args[0]));
